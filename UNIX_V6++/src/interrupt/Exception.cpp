...
void Exception::PageFault(struct pt_regs* regs, struct pte_context* context)
{
	...
	Diagnose::Write("esp: %x\n ", context->esp);
	Diagnose::Write("Text: %x,%x,  Data: %x,%x, stack size: %x\n ", md.m_TextStartAddress,md.m_TextSize,
	...
	if( (context->xcs & USER_MODE) == USER_MODE)
	{
		if( cr2 < MemoryDescriptor::USER_SPACE_SIZE - md.m_StackSize && cr2 >= context->esp - 8
				&& md.m_DataSize + md.m_StackSize + 4096 < MemoryDescriptor::USER_SPACE_SIZE - md.m_DataStartAddress )
			current->SStack();
		else
		{
