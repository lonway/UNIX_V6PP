...
bool MemoryDescriptor::EstablishUserPageTable( unsigned long textVirtualAddress, unsigned long textSize, unsigned long dataVirtualAddress, unsigned long dataSize, unsigned long stackSize )
{
	...
	if ( textSize + dataSize + stackSize  + 4096 > USER_SPACE_SIZE - textVirtualAddress)
	{
...
void MemoryDescriptor::MapToPageTable()
{
	...
	/*
	pUserPageTable[1].m_Entrys[1023].m_Present = 1;
	pUserPageTable[1].m_Entrys[1023].m_ReadWriter = 1;
	pUserPageTable[1].m_Entrys[1023].m_PageBaseAddress = (u.u_procp->p_addr + this->m_DataSize + this->m_StackSize)>> 12;
    */
